...
// ── Hero — mosaic de posters en arrière-plan ──────────────────────────────────
...
function HeroMosaic() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
      {/* Grille de posters en perspective */}
      <div
      ...
      <div className="relative overflow-hidden rounded-[16px] border border-white/[0.09] bg-white/[0.025] shadow-[0_32px_80px_rgba(0,0,0,0.6)] backdrop-blur-sm">
        ...
        <div className="flex items-center gap-3 border-b border-white/[0.07] bg-white/[0.03] px-4 py-3">
          ...
          <div className="mx-auto flex h-6 items-center gap-1.5 rounded-full bg-white/[0.05] px-4">
            ...
            <span className="text-[10px] text-text-secondary/60">swipefilm.app</span>
          </div>
        </div>
        ...
        <div className="relative h-28 overflow-hidden bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f3460]">
          ...
          <div className="absolute bottom-4 left-6">
            ...
            <p className="font-display text-base font-bold text-text-primary">The Prestige</p>
          </div>
          <span className="absolute top-3 right-4 rounded-[4px] bg-secondary px-1.5 py-0.5 text-[9px] font-bold text-bg-primary">94%</span>
        </div>

        {/* Rangées de recommandations */}
        <div className="space-y-4 bg-bg-primary p-4 pb-5">
          {[
            { label: "Pour vous", offset: 0 },
            { label: "Trésors cachés", offset: 5 },
            { label: "Parce que tu as aimé The Prestige", offset: 10 },
          ].map((row) => (
            <div key={row.label}>
              <p className="mb-2 px-1 text-[10px] font-semibold text-text-primary">{row.label}</p>
              <div className="flex gap-1.5 overflow-hidden">
                {Array.from({ length: 6 }).map((_, i) => {
                  const [from, to] = SECTION_COLORS[(row.offset + i) % SECTION_COLORS.length];
                  return (
                    <div
                      key={i}
...
// ── Demo cards (mockup swipe) ─────────────────────────────────────────────────

const DEMO_CARDS = [
  { title: "The Prestige", year: "2006", director: "Nolan", duration: "130 min", score: "94", genres: ["Thriller", "Mystère"], bg: "from-[#0d1b4b] via-[#162555] to-[#1e3a8a]" },
  { title: "Spirited Away", year: "2001", director: "Miyazaki", duration: "125 min", score: "97", genres: ["Animation", "Fantastique"], bg: "from-[#0d3b2c] via-[#135e46] to-[#1a7a5e]" },
  { title: "No Country for Old Men", year: "2007", director: "Coen Bros", duration: "122 min", score: "91", genres: ["Thriller", "Crime"], bg: "from-[#3b1a05] via-[#6b2d08] to-[#8b3a0a]" },
];
...
function SwipeDeck() {
  ...
  return (
    <div className="relative flex justify-center select-none pb-16" aria-hidden>
      <div className="absolute left-5 top-5 h-[332px] w-52 rounded-[16px] border border-white/[0.07] bg-white/[0.02] opacity-30 rotate-3" />
      <div className="absolute left-2.5 top-2.5 h-[332px] w-52 rounded-[16px] border border-white/[0.06] bg-white/[0.015] opacity-55 rotate-1" />
      <div style={{ animation: anim }}>
        <div className="w-52 overflow-hidden rounded-[16px] border border-white/[0.09] bg-white/[0.03] shadow-2xl backdrop-blur-sm">
          <div className={`relative h-64 bg-gradient-to-br ${card.bg}`}>
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-bg-primary/95 via-bg-primary/50 to-transparent px-4 pb-3 pt-8">
              <p className="font-display text-sm font-bold leading-snug text-text-primary">{card.title}</p>
            </div>
          </div>
          <div className="p-3.5">
            <p className="text-xs text-text-secondary">{card.year} · {card.director} · {card.duration}</p>
...
function IcoSwipe() {
  return <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M7.5 21 3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" /></svg>;
}
function IcoSparkle() {
...
function IcoCheck() {
  return <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>;
}
function IcoX() {
  return <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>;
}
        ...
        <section className="relative overflow-hidden pb-0 pt-24">
          ...
          <div className="relative mx-auto max-w-4xl px-6 text-center">
            ...
            <p className="mx-auto mt-5 max-w-lg text-lg leading-relaxed text-text-secondary">
              SwipeFilm connecte ton <strong className="text-text-primary">Jellyfin</strong> ou
            ...
            {/* Stats prominentes (comme seerr.dev) */}
            <div className="mt-12 flex flex-wrap items-center justify-center gap-8 md:gap-14">
              {[
                ...
                <div key={s.label} className="text-center">
                  <p className={`font-display text-3xl font-bold tabular-nums ${s.highlight ? "text-secondary" : "text-text-primary"}`}>
                    {s.value}
                  </p>
          ...
          {/* ✦ App mockup (comme seerr.dev) */}
          <AppMockup />
        </section>

        {/* ── INTÉGRATIONS (comme seerr.dev) ── */}
        <section className="py-16">
          <FadeIn className="mx-auto max-w-6xl px-6 text-center">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              {[
                { name: "Jellyfin", color: "#00A4DC", letter: "J" },
                { name: "Plex", color: "#E5A00D", letter: "P" },
                { name: "Seerr", color: "#4a9eda", letter: "S" },
                { name: "TMDB", color: "#01D277", letter: "T" },
                { name: "Radarr", color: "#e2a100", letter: "R" },
                { name: "Sonarr", color: "#35c5f4", letter: "S" },
              ].map((p) => (
                <div key={p.name} className="flex flex-col items-center gap-2">
                  <div
                    className="flex h-14 w-14 items-center justify-center rounded-[14px] border border-white/[0.08] bg-white/[0.03] text-lg font-bold backdrop-blur-sm transition-colors hover:bg-white/[0.07]"
                    style={{ color: p.color }}
                    aria-label={p.name}
                  >
                    {p.letter}
            ...
            <div className="grid items-center gap-12 lg:grid-cols-2">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { icon: <IcoFilm />, title: "Tes fichiers restent sur ton serveur", desc: "SwipeFilm ne stocke que les métadonnées TMDB et tes swipes.", color: "text-secondary border-secondary/20", delay: 300 },
                  ...
                  <FadeIn key={item.title} delay={item.delay as 100 | 200 | 300 | 400}>
                    <div className="flex gap-4 rounded-[12px] border border-white/[0.07] bg-white/[0.025] p-4 backdrop-blur-sm transition-colors hover:bg-white/[0.05]">
                      <div className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-[8px] border bg-white/[0.03] ${item.color}`}>
                        {item.icon}
                      </div>
                      <div>
        ...
        <section className="bg-surface py-20">
          <div className="mx-auto max-w-6xl px-6">
            <div className="grid gap-5 md:grid-cols-3">
              {[
                { icon: <IcoServer />, title: "Ton serveur, tes règles", desc: "SwipeFilm lit ta bibliothèque Jellyfin ou Plex. Pas de cloud, pas de données tierces. Tout reste chez toi.", color: "text-secondary border-secondary/20", delay: 100 },
